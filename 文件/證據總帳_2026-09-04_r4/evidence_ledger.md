...
- `文件/身份特徵是盲點製造機_2026-09-03.md` — 6740 bytes, SHA-256 `b60ae44ff8c3807f08a1127cb6e379119fac161acfee3b0feaa4e17b0a04a74e` (Six leave-out configurations, the mechanism, and the retraction of the 2026-09-01 recommendation)
...
- Corrected on 2026-09-04: the cited document originally said the measurement used the validation partition only. The tool has no split filter, so test-labelled rows were in the training set. The conclusion stands (held-out classes are excluded whole, nothing is tuned, the evaluation set is entirely unseen) but the description was wrong.
- The identity channel was measured as a pair, but sros_permission_deny_rate is a constant-zero column on this table. The measured cost comes entirely from sros_auth_fail_rate.
...
### Evidence for `defence_reaction_feature_audit`
